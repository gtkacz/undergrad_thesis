...
def train_and_evaluate_model(
	...
	for epoch in range(num_epochs):
		...
		if verbose:
			print(f"Epoch [{epoch + 1}/{num_epochs}], Loss: {avg_loss:.4f}, Validation Accuracy: {accuracy:.4f}")
...
def evaluate(
	...
	for epoch in range(num_epochs):
		...
		for images, labels in train_loader:
			...
			preds = (outputs > 0.5).float()  # Predictions: apply threshold to get binary output
			training_running_corrects += torch.sum(preds == labels).item()  # Count correct predictions
			total_train_samples += labels.size(0)  # Count total samples

		# Compute metrics for the epoch
		train_loss = running_loss / len(train_loader)  # Average training loss
		train_accuracy = training_running_corrects / total_train_samples  # Training accuracy
		...
		with torch.no_grad():  # Disable gradient calculation
			for images, labels in validation_loader:
				...
				preds = (outputs > 0.5).float()  # Predictions: apply threshold to get binary output
				val_running_corrects += torch.sum(preds == labels).item()  # Count correct predictions
	...
	with torch.no_grad():  # Disable gradient tracking to save memory and computation
		for images, labels in test_loader:
			...
			outputs = outputs.squeeze()  # Remove extra dimensions from the output
			preds = (outputs > 0.5).float()  # Apply threshold to get binary predictions (0 or 1)
			...
			test_running_corrects += torch.sum(preds == labels).item()  # Compare predicted and true labels
			...
			all_labels.extend(labels.cpu().numpy())  # Move true labels to CPU and convert to numpy array
